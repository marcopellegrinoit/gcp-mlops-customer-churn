...
### The daily job scores one snapshot, not the whole mart

`bigquerySource.inputUri` takes a table reference and accepts no row filter, so restricting the daily `BatchPredictionJob` to the current snapshot has to be expressed upstream. **`features.customer_features_current`** is a dbt model that materialises the newest partition of `customer_features` (minus the label and build-metadata timestamps), rebuilt by the same `dbt run` that builds the mart — so it needs no separate orchestration step and is always in step with its source.

Pointing the job at `customer_features` itself meant re-scoring every retained partition every night: **246,321 rows to produce 34,495 genuinely new predictions**, and the mart retains ~90 daily partitions, so that ratio only worsens.

The cost was the lesser problem. `sync_predictions_to_ml_predictions` MERGEs on `(customer_id, snapshot_date)`, so re-scoring history *overwrote* it — and `ml.predictions` declares `predicted_at` as "when this prediction was written" and `model_version` as "the champion used to score this row". Both were restamped nightly, so August's rows claimed to have been predicted today. Nothing contradicted itself only because a single champion has ever existed; the first promotion would have silently rewritten every retained day with a model that did not exist on those days, destroying the answer to "what did we tell the business that day".

An operational prediction log is a point-in-time record and must be append-only in effect. Re-scoring history under an improved model is a legitimate thing to want — for consistent backtests — but it belongs in a separate, model-versioned table, not in place in the log.

The trade this accepts: historical predictions are now immutable, so a future serving defect cannot be retro-repaired the way the `instanceType` defect below was. That is the right trade now that prediction-score drift catches that class of failure within a day.

Note that `customer_features_current` deliberately drops `churned`. With `instanceType: "object"` every column present is serialised into the instance sent to the serving container; `select_inference_features` discards anything outside the frozen feature set, but the target should not be shipped into a scoring request on the assumption that it will be ignored. `customer_id` and `snapshot_date` are kept — the batch job echoes source columns into its output table and the MERGE keys on exactly that pair.
