...
| `drift-monitor` | MLE | PSI drift check of the latest feature snapshot against the champion's training baseline (depends on `ml-common` for `drift.py`/`preprocess.py`) | Yes — runs as `drift-monitor-job`, a Cloud Run Job |
...
Vertex AI requires a custom-container `BatchPredictionJob`'s `bigqueryDestination` to reference a *dataset*, not a specific table — it auto-creates its own `predictions_<timestamp>` table inside `ml`. Reconciling that output into the fixed `ml.predictions` schema (joining back `customer_id`, stamping `model_version`/`predicted_at`) is not yet built; the daily batch-predict step currently exists to keep the champion warm and to match the documented DAG, but a follow-up ETL step is needed before `ml.predictions` is actually populated end-to-end.
