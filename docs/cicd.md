...
| `uv-lock (workspace)` | root `pyproject.toml`/`uv.lock` | Re-locks the workspace when a manifest changes, so `uv sync --frozen` in the Dockerfiles never fails on a stale `uv.lock` |
| `uv-lock (dbt_transform)` | `projects/dbt_transform/pyproject.toml`/`uv.lock` | Same guarantee for the second lock. A separate entry is required: the upstream hook's `files` pattern is root-anchored, so it never fires on a dbt-only dependency change ([why that lock is separate](architecture.md#why-dbt_transform-is-not-a-workspace-member)) |
...
| `dashboard-trigger` | `projects/obs_common/**`, `projects/dashboard/**`, `projects/dbt_transform/models/marts/**`, `pyproject.toml`, `uv.lock` | `.cloudbuild/dashboard.cloudbuild.yaml` |
...
`dashboard-trigger` is the only one watching another project's source: it includes `projects/dbt_transform/models/marts/**` because the dashboard's marts are the contract between dbt and the app, and one of its tests asserts that the high-risk floor duplicated in `churn_risk_daily.sql` still matches the app's default (see [dashboard.md](dashboard.md#risk-bands-and-the-duplicated-threshold)). A change to the marts has to re-run that check.
...
4. **Programmatic Pipeline Compilation:** `training-pipeline-trigger` fires two ways: directly, when `projects/training_pipeline/**` changes, or chained, as the final `trigger-pipeline-compile` step of `trainer-trigger`, `post-training-trigger`, and `serving-trigger` (`gcloud builds triggers run training-pipeline-trigger`). Because Cloud Build steps run sequentially and stop on first failure, that chaining step is only reached after a component's `test` → `build` → `push` steps all succeed — a broken image build never triggers a pipeline recompile. This is one-directional decoupling: editing the pipeline doesn't rebuild/push an unchanged image, but a verified trainer/post-training/serving rebuild does trigger a recompile.
   ...
   The same step also carries the model policy. Its `env` block exports the `ML_*`/`MODELING_*` variables (promotion gate deltas, target recall, rejection limit, HPO trial and fold budget) from `_ML_*`/`_MODELING_*` substitutions, which the deployed trigger sets from `iac/config/triggers.yaml`. `training_pipeline.settings_env` resolves them through the same `pydantic-settings` classes the containers use and pins the result onto every task's container spec — so retuning the gate is a Terraform apply plus a recompile, and the policy a given run was judged by is recorded in the pipeline template rather than inferred from whatever the images happened to default to. See [architecture.md](architecture.md#data-contracts--configuration).
...
`dashboard-trigger` is the only pipeline that ends in a `gcloud run services update` — a deploy step, not just a push. The Cloud Run Jobs are updated the same way (`gcloud run jobs update`), but the dashboard is the one deployable serving live traffic, so a successful build puts the new revision in front of users immediately. `roles/run.developer`, already on the default Cloud Build SA, covers it. It also has no `integration-test` step: the container reads exactly one external service (BigQuery, read-only) and the emulator cannot execute the views the app reads, so an emulator-backed test would assert that a mock returns what the mock was told to. Its query layer's real contract — the byte ceiling and the table addresses — is unit-tested instead, and `tests/test_app.py` renders the actual page headlessly through Streamlit's `AppTest` harness.
