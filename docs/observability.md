...
At check time, `drift_monitor.predictions.fetch_latest_predictions` finds the champion's most recent **successfully completed, daily-scoring** `BatchPredictionJob` — disambiguated from `post_training`'s own evaluation-time batch prediction against the champion by `display_name` (`BATCH_PREDICT_DISPLAY_NAME` env var, must match `submit_batch_predict`'s `displayName` in `orchestrator-workflow.yaml`) — and reads its raw BigQuery output table (`output_info.bigquery_output_dataset` + `bigquery_output_table`). This reads Vertex's unmanaged, auto-created `predictions_<timestamp>` table directly rather than the clean `ml.predictions` schema declared in `iac/config/bigquery.yaml`, since nothing reconciles batch-predict output into that table yet (a separate, larger piece of work).
