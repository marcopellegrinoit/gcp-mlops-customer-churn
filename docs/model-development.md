...
The threshold is selected during the **train** pipeline stage, not evaluate, and never from the held-out test set. `modeling.train.select_threshold_via_cv` refits the already-chosen hyperparameters across the same `n_folds` stratified folds used by HPO (no search this time, just refitting), collects each fold's held-out (out-of-fold) predictions into one probability array spanning the whole training set, and selects the lowest threshold value achieving a **target recall of ≥ 0.80** on those out-of-fold predictions via `ml_common.evaluate.select_threshold`. If multiple thresholds satisfy that constraint, the one with the highest precision among them is chosen to limit the volume of false-alarm interventions.
