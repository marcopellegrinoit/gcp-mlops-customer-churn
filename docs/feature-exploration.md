...
On every subsequent training run, the `evaluate` stage computes the **Spearman rank correlation** between the current run's SHAP ranking and the stored baseline (`ml_common.evaluate._spearman_rank_correlation`). Ties are handled explicitly rather than assumed away: XGBoost gives a feature it never split on a mean |SHAP| of exactly 0.0, so a model with unused features carries a block of exact ties, and the two models being compared rarely leave the same features unused. The implementation therefore uses average ranks and Pearson over those ranks — matching `scipy.stats.spearmanr` — rather than the `1 - 6·Σd²/(n(n²−1))` shortcut, which is only equal to Spearman when nothing is tied. It returns `None` when either side's importances are all equal, since a constant vector has no ordering to correlate against. If this correlation drops below **0.75**, feature relevance has shifted significantly — a signal to open the exploration notebook. The score travels through `register_or_reject` onto the outcome payload as `shap_rank_correlation` and is logged by `notify`.
