...
3. **Batch Prediction:** Once feature availability is confirmed and a champion model is registered, the workflow submits a Vertex AI `BatchPredictionJob` against the champion, polling it to completion. If no champion is registered yet (first-ever run), the workflow stops here cleanly — there's nothing to score or monitor yet.
   ...
   Batch prediction needs dedicated resources, and Vertex will fail the job outright with *"Machine type temporarily unavailable, please deploy with a different machine type or retry"* when the region is short on capacity for the type requested — a job-level failure, not an HTTP error, so the submit call's own retry policy never sees it. `submit_batch_predict` catches that state and resubmits up to three times, **rotating the machine type** (`n1-standard-4` → `n2-standard-4` → `e2-standard-4`, then wrapping) rather than retrying an identical spec: capacity is tracked per machine type, so a retry that changes nothing is the weaker half of what the error actually asks for. All three are 4 vCPU / ≥15 GB, so the fallbacks cost and perform the same. Only after the whole ladder fails does the run alert and halt.
