...
def compute_baseline_stats(X: pd.DataFrame) -> dict[str, BaselineSpec]:
    """Freeze a per-feature baseline distribution from training data for later PSI comparison."""
    stats: dict[str, BaselineSpec] = {}
...
def compute_psi(baseline_stats: Mapping[str, object], current: pd.DataFrame) -> dict[str, float]:
    """Return the PSI score for every baseline feature present in current."""
    scores = {}
    for col, spec in parse_baseline_stats(baseline_stats).items():
...
def psi_noise_floor(spec: BaselineSpec, n_current: int) -> float:
    ...
    """
    expected_pct = np.asarray(spec.expected_vector, dtype=float)
...
def evaluate_drift(
    baseline_stats: Mapping[str, object], current: pd.DataFrame, psi_threshold: float = 0.2
) -> DriftDecision:
    ...
    null_rates: dict[str, NullRateComparison] = {}
    ...
    for col, spec in parse_baseline_stats(baseline_stats).items():
        ...
        if spec.tracks_nulls:
            null_rates[col] = NullRateComparison(
                baseline=float(spec.null_rate), current=_null_rate(current[col])
            )

        if not spec.is_monitored:
...
def _categorical_baseline(values: pd.Series) -> CategoricalBaseline:
...
def _numeric_baseline(values: pd.Series) -> DiscreteBaseline | NumericBaseline:
    ...
    if clean.empty:
        ...
        return DiscreteBaseline(frequencies={}, null_rate=null_rate, monitored=False)

    if clean.nunique() <= _MAX_DISCRETE_CARDINALITY:
        return DiscreteBaseline(
...
def _distributions(spec: BaselineSpec, current: pd.Series) -> tuple[np.ndarray, np.ndarray]:
    """Align the baseline and the live column onto a shared set of buckets."""
    tracks_nulls = spec.tracks_nulls
...
def _append_null_bucket(
    spec: BaselineSpec, expected_pct: np.ndarray, actual_pct: np.ndarray, current: pd.Series
...
def _binned_distributions(
    spec: NumericBaseline, current: pd.Series
) -> tuple[np.ndarray, np.ndarray]:
    ...
    expected_pct = np.asarray(spec.value_proportions, dtype=float)
